...
- **Supported NGO pilot readiness:** 94-96% after external email/Auth configuration and real-inbox verification
- **Broad public self-serve NGO readiness:** 85-89%
- **Core NGO product workflow completeness:** 91-94%

The product loop is mostly built. Since the original June 2 readiness pass, repo-side live email/Auth closure has been completed and documented in `docs/ngo-live-email-auth-closure-result.md`. The remaining non-Stripe gaps are now external Vercel/Supabase/Resend configuration, real-inbox verification, file-scanning/manual review maturity, final browser/mobile proof, and operational/external review.
...
- Repo-side live email/Auth redirect handling, password-reset hash bridging, and plain-language auth copy are complete.
...
Status: **repo-side closure complete; external setup and real-inbox proof still required**

Required:

- Use `docs/ngo-live-email-auth-closure-result.md` as the source of truth for exact Vercel, Supabase, and Resend setup steps.
- Verify Resend sending domain.
- Configure Supabase Auth custom SMTP for the clean V2 project.
- Add/verify required Vercel env vars for Resend/support email.
...
1. External live email/Auth setup: Vercel env, Supabase SMTP, Resend domain/sender.
...
- External email delivery and password reset have not been proven end to end with real inboxes.
...
1. Complete external live email/Auth setup and real-inbox verification.
...
- `node --test scripts/ngo-near-100-non-stripe-readiness.test.mjs` - passed, 2/2 when originally created; rerun after this freshness update is recommended before commit.
- `npm test` - latest full suite passed, 161/161 during the June 16 email/Auth closure and pilot-readiness verification runs.
...
- `npm run build` - started `next build` during the June 16 email/Auth closure run but produced no output for more than one minute and was stopped as a local tooling stall.
